...
#include "debuggerd/libdebuggerd/gen/tombstone.pb.h"
...
void set_human_readable_cause(Cause* cause, uint64_t fault_addr) {
  if (!cause->memory_error || !cause->memory_error->heap) {
    return;
  }
  ...
  switch (memory_error.tool) {
    case MemoryErrorTool::GWP_ASAN:
      tool_str = "GWP-ASan";
      break;
    case MemoryErrorTool::SCUDO:
      tool_str = "MTE";
      break;
    default:
      tool_str = "Unknown";
  ...
  switch (memory_error.type) {
      case MemoryErrorType::USE_AFTER_FREE:
      error_type_str = "Use After Free";
      break;
    case MemoryErrorType::DOUBLE_FREE:
      error_type_str = "Double Free";
      break;
    case MemoryErrorType::INVALID_FREE:
      error_type_str = "Invalid (Wild) Free";
      break;
    case MemoryErrorType::BUFFER_OVERFLOW:
      error_type_str = "Buffer Overflow";
      break;
    case MemoryErrorType::BUFFER_UNDERFLOW:
  ...
  cause->human_readable = (StringPrintf(
...
void dump_stack_history(unwindstack::AndroidUnwinder* unwinder, uintptr_t target_tls,
  ...
  for (uintptr_t idx = 16; idx <= buf_size; idx += 16) {
    ...
    shb_obj.entries.emplace_back();
    StackHistoryBufferEntry* entry = &shb_obj.entries.back();
    fill_in_backtrace_frame(&entry->addr, frame_data);
    entry->fp = (fp);
    entry->tag = (tag);
  }
}
...
static std::optional<std::string> maybe_stack_mte_cause(
  ...
  if (it != threads.end()) {
    StackHistoryBuffer* shb = tombstone->stack_history_buffer;
    shb->tid = (tid);
...
static void dump_probable_cause(Tombstone* tombstone, unwindstack::AndroidUnwinder* unwinder,
  ...
  if (cause) {
    tombstone->causes.emplace_back();
    Cause *cause_proto = &tombstone->causes.back();
    cause_proto->human_readable = (*cause);
  }
}
...
static void dump_crash_details(Tombstone* tombstone,
  ...
  while (address) {
  async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "%lu", address);
    ...
    for (size_t i = 0; i < page.used; ++i) {
      ...
      tombstone->crash_details.emplace_back();
      auto* proto_detail = &tombstone->crash_details.back();
      proto_detail->name = (name);
      proto_detail->data = (data);
    }
...
static void dump_open_fds(Tombstone* tombstone, const OpenFilesList* open_files) {
  if (open_files) {
    for (auto& [fd, entry] : *open_files) {
      ...
      tombstone->open_fds.push_back(f);
    }
  }
}
...
void fill_in_backtrace_frame(BacktraceFrame* f, const unwindstack::FrameData& frame) {
  ...
  if (!frame.function_name.empty()) {
    ...
    if (demangled_name) {
      f->function_name = (demangled_name);
      free(demangled_name);
    } else {
      f->function_name = (frame.function_name);
    }
  }
  ...
  if (!frame.map_info->name().empty()) {
    f->file_name = (frame.map_info->GetFullName());
  } else {
    f->file_name = (StringPrintf("<anonymous:%" PRIx64 ">", frame.map_info->start()));
  }
  f->file_map_offset = (frame.map_info->elf_start_offset());

  f->build_id = (frame.map_info->GetPrintableBuildID());
}
...
static void dump_registers(unwindstack::AndroidUnwinder* unwinder,
  ...
  regs->IterateRegisters([&thread, memory_dump, maps, memory](const char* name, uint64_t value) {
    ...
    r.name = (name);
    r.u64 = (value);
    thread.registers.push_back(r);

    if (memory_dump) {
      MemoryDump dump;

      dump.register_name = (name);
      std::shared_ptr<unwindstack::MapInfo> map_info = maps->Find(untag_address(value));
      if (map_info) {
        dump.mapping_name = (map_info->name());
      }
      ...
      dump.begin_address = (value);
      dump.memory = std::vector<uint8_t>(buf, buf + bytes);
      ...
      if (has_tags) {
        dump.arm_mte_metadata->memory_tags = std::basic_string<char>(tags, tags + kNumTagsAroundRegister);
      }

      thread.memory_dump.push_back(std::move(&dump));
    }
  });
}
...
static void dump_thread_backtrace(std::vector<unwindstack::FrameData>& frames, Thread& thread) {
  ...
  for (const auto& frame : frames) {
    thread.current_backtrace.emplace_back();
    BacktraceFrame* f = &thread.current_backtrace.back();
  ...
  if (!unreadable_elf_files.empty()) {
    auto unreadable_elf_files_proto = thread.unreadable_elf_files;
    auto backtrace_note = thread.backtrace_note;
    backtrace_note.emplace_back(
        "Function names and BuildId information is missing for some frames due");
    backtrace_note.emplace_back("to unreadable libraries. For unwinds of apps, only shared libraries");
    backtrace_note.emplace_back("found under the lib/ directory are readable.");
    backtrace_note.emplace_back("On this device, run setenforce 0 to make the libraries readable.");
    backtrace_note.emplace_back("Unreadable libraries:");
    for (auto& name : unreadable_elf_files) {
      backtrace_note.emplace_back("  " + name);
      unreadable_elf_files_proto.emplace_back(name);
    }
  }
}
...
static void dump_thread(Tombstone* tombstone, unwindstack::AndroidUnwinder* unwinder,
  ...
  thread.id = (thread_info.tid);
  thread.name = (thread_info.thread_name);
  thread.tagged_addr_ctrl = (thread_info.tagged_addr_ctrl);
  thread.pac_enabled_keys = (thread_info.pac_enabled_keys);
  ...
  auto& threads = tombstone->threads;
  ...
  if (guest_unwinder) {
    ...
    auto& guest_threads = tombstone->guest_threads;
...
static void dump_mappings(Tombstone* tombstone, unwindstack::Maps* maps,
  ...
  for (const auto& map_info : *maps) {
    tombstone->memory_mappings.emplace_back();
    auto* map = &tombstone->memory_mappings.back();
    map->begin_address = (map_info->start());
    map->end_address = (map_info->end());
    ...
    map->mapping_name = (map_info->name());
    ...
    map->load_bias = (map_info->GetLoadBias(process_memory));
  }
}
...
static void add_error_log_msg(Tombstone* tombstone, const std::string&& error_msg) {
  ...
  buffer.logs.emplace_back();
  LogMessage* log_msg = &buffer.logs.back();
  log_msg->timestamp = ("00-00 00:00:00.000");
  log_msg->pid = (0);
  log_msg->tid = (0);
  log_msg->priority = (ANDROID_LOG_ERROR);
  log_msg->tag = ("");
  log_msg->message = (error_msg);

  tombstone->log_buffers.push_back(std::move(buffer));
...
static void dump_log_file(Tombstone* tombstone, const char* logger, pid_t pid) {
  ...
  while (true) {
    ...
    auto sec = static_cast<time_t>(log_entry.entry.sec);
    tm tm{};
    localtime_r(&sec, &tm);
    ...
    do {
      ...
      log_msg->pid = (log_entry.entry.pid);
      log_msg->tid = (log_entry.entry.tid);
      log_msg->priority = (prio);
      log_msg->tag = (tag);
      log_msg->message = (msg);
    } while ((msg = nl));
  }
...
static void dump_tags_around_fault_addr(Signal* signal, const Tombstone& tombstone,
  ...
  tag_dump.begin_address = (start_address);

  std::string* mte_tags = &tag_dump.arm_mte_metadata->memory_tags;
  ...
  if (!mte_tags->empty()) {
    *signal->fault_adjacent_metadata = tag_dump;
  }
}
...
void engrave_tombstone_proto(Tombstone* tombstone, unwindstack::AndroidUnwinder* unwinder,
  ...
  if (guest_arch != nullptr) {
    result.guest_arch = (*guest_arch);
  } else {
    result.guest_arch = (Architecture::NONE);
  }
  /*result.build_fingerprint = (android::base::GetProperty("ro.build.fingerprint", "unknown"));
  result.revision = (android::base::GetProperty("ro.revision", "unknown"));*/
  result.build_fingerprint = ("unknown");
  result.revision = ("unknown");
  result.timestamp = (get_timestamp());

  const ThreadInfo& target_thread = threads.at(target_tid);
  result.pid = (target_thread.pid);
  result.tid = (target_thread.tid);
  result.uid = (target_thread.uid);
  result.selinux_label = (target_thread.selinux_label);
  ...
  if (android::procinfo::GetProcessInfo(target_thread.pid, &proc_info, &error)) {
    ...
    result.process_uptime = (si.uptime - starttime);
  } else {
  ...
  // result.has_been_16kb_mode = (android::base::GetBoolProperty("ro.misctrl.16kb_before", false));
  result.has_been_16kb_mode = false;

  auto cmd_line = result.command_line;
  ...
  Signal sig;
  sig.number = (target_thread.signo);
  sig.name = (get_signame(target_thread.siginfo));
  sig.code = (target_thread.siginfo->si_code);
  sig.code_name = (get_sigcode(target_thread.siginfo));

  if (signal_has_sender(target_thread.siginfo, target_thread.pid)) {
    sig.has_sender = (true);
    sig.sender_uid = (target_thread.siginfo->si_uid);
    sig.sender_pid = (target_thread.siginfo->si_pid);
  }

  if (process_info.has_fault_address) {
    sig.has_fault_address = (true);
    uintptr_t fault_addr = process_info.maybe_tagged_fault_address;
    sig.fault_address = (fault_addr);
    dump_tags_around_fault_addr(&sig, result, unwinder->GetProcessMemory(), fault_addr);
  }
    async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "c");

  result.signal_info = new Signal();
  *result.signal_info = sig;
  async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "%lu", sig.fault_address);
