...
static void print_backtrace(CallbackType callback, SymbolizeCallbackType symbolize,
                            ...
                            const std::vector<BacktraceFrame>& backtrace,
                            bool should_log) {
  int index = 0;
...
static void print_thread_memory_dump(CallbackType callback, const Tombstone& tombstone,
  ...
  int word_size = pointer_width(tombstone);
  for (const auto& _mem : thread.memory_dump) {
      const auto mem = *_mem;
    CBS("");
...
static void print_tag_dump(CallbackType callback, const Tombstone& tombstone) {
  if (!tombstone.signal_info) return;

  const Signal& signal = *tombstone.signal_info;

  if (!signal.has_fault_address || !signal.fault_adjacent_metadata) {
    return;
  }

  const MemoryDump& memory_dump = *signal.fault_adjacent_metadata;

  if (!memory_dump.arm_mte_metadata || memory_dump.arm_mte_metadata->memory_tags.empty()) {
    return;
  }
...
static void print_main_thread(CallbackType callback, SymbolizeCallbackType symbolize,
  ...
  if (tombstone.stack_history_buffer) {
  ...
  for (const Cause& cause : tombstone.causes) {
    ...
    if (cause.memory_error && cause.memory_error->heap) {
      ...
      if (heap_object.deallocation_backtrace.size() != 0) {
        CBS("");
      ...
      if (heap_object.allocation_backtrace.size() != 0) {
        CBS("");
